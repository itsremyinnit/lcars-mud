...
#define DEFAULT_LOG_DIR "/d/Conf/logs/"
...
#define MEETING_HELP "/d/Conf/text/meeting.help"
#define MODERATE_HELP "/d/Conf/text/moderate.help"
#define RULES_HELP "/d/Conf/text/rules.help"
...
// commands even when he has the floor. Alas.
	...
	logdir = DEFAULT_LOG_DIR ;
        ...
        // Changed from simple variable assignment by Blue
        // to make sure the echo room loads.  Otherwise
        // there's trouble.
...
int emote (string str) {
    ...
    foo = iwrap(first + this_player()->query("cap_name") + insert + str) ;
    write (foo) ;
    say (foo) ;
    if (logging) write_file (logdir+logfile,foo) ;
    if (echoing) tell_room (echo_room , "Conf: "+foo) ;
...
int comment (string str) {
	...
	foo = iwrap((string)this_player()->query("cap_name") +" comments: " + str) ;
	say (foo) ;
	if (logging) write_file (logdir+logfile, foo) ;
	if (echoing) tell_room (echo_room , "Conf: "+foo) ;
        write(iwrap("You say: " + str));
...
int call_for_vote (string str) {
	...
	if (getuid(this_player())!=moderator) {
		notify_fail ("Only the moderator may call for votes.\n") ;
		return 0 ;
	}
...
// This is a roll call vote so we log the votes and announce them.
	...
	write_file (logdir+votefile,"\n"+iwrap(capitalize(moderator)+" calls for a roll call vote on "+vote_str)) ;
	say (capitalize(moderator)+" sets the clock to "+str+".\n") ;
	endtime = time()+i ;
        call_out ("expire_vote",i) ;
        return 1 ;
...
// Added by Blue 950423 so the echo room can tell if it's needed.
