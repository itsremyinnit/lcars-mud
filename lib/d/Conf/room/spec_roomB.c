...
void create() {
	...
	seteuid(getuid()) ;
	set ("light", 1) ;
	set ("short", "Spectator Room B") ;
	set ("long",
"This is the observation room for meeting room B. There is a large window\n"+
...
int read (string str) {
	...
	ob = find_object_or_load("/d/Conf/room/room_B") ;
	ob->read_sign("sign") ;
	return 1 ;
}
...
int look_at_room() {
	...
	env = find_object_or_load("/d/Conf/room/room_B.c") ;
...
int clean_up() {
    object watchroom;
    if (!(watchroom=find_object("/d/Conf/room/room_B.c"))) {
        return ::clean_up();
    }
